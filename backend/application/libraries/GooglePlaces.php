...
require APPPATH . '/libraries/googleGeocoding.php';
...
class GooglePlaces {
    ...
    protected function _executeAPICall()
    {
        ...
        //log_message('info','URLToCall --> ' .$URLToCall);
        $result = json_decode($this->_curlCall($URLToCall), true);

        //log_message('info','result --> ' .print_r($result));


        $formattedResults = $this->_formatResults($result);
        //log_message('info','formattedResults --> ' .var_dump($formattedResults));
    ...
    protected function _formatResults($result) {
        ...
        //log_message('info','result result--> ' .var_dump($result));
